// lib/features/providers/offer_provider.dart - WITH DEVICE LIMIT
import 'dart:convert';
...
class OffersProvider extends ChangeNotifier {
  ...
  /// Initialize with device discount manager
  ...
  /// Release the current discount (e.g., after order completion)
  ...
  Future<void> loadOffers() async {
    ...
    try {
      // Ensure device discount manager is initialized
      if (_deviceDiscountManager == null) {
        await initialize();
      }
  ...
  /// Load main offers with platform filter
  Future<void> _loadMainOffers() async {
    try {
      final platform = _getPlatform();
      
      final response = await http.get(
        Uri.parse('$baseUrl/offer?platform=$platform'),
        headers: {
  ...
  /// Load items with active offers (with platform filter)
  Future<void> _loadItemsWithOffers() async {
    try {
      final platform = _getPlatform();
      final url = '$baseUrl/offer/items-with-offers?platform=$platform&includeUnavailable=false';
